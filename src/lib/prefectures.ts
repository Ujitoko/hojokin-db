export const PREFECTURES = [
  ...
  { slug: "okinawa", name: "沖縄県", region: "九州" },
] as const;

export type Prefecture = (typeof PREFECTURES)[number];

export function findPrefecture(text: string): Prefecture | undefined {
  return PREFECTURES.find((p) => text.includes(p.name.replace("県", "").replace("府", "").replace("都", "").replace("道", "")) || text.includes(p.name));
}
